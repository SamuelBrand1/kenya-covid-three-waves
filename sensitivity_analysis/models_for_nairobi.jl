...
AIC_two_group  = -2*maximum(nai_two_group.MCMC_results.logL) + 2*(size(nai_two_group.MCMC_results.chain,2))
BIC_two_group = -2*maximum(nai_two_group.MCMC_results.logL) + log(sum(nai_two_group.PCR_cases) + sum(nai_two_group.sero_cases))*(size(nai_two_group.MCMC_results.chain,2))
...
res = bboptimize(f; SearchRange = searchrange,PopulationSize=500,MaxSteps=5000)
